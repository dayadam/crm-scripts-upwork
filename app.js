...
async function scrape(searchTerm, res) {
  ...
  //get record url
  const recordURL = await page.evaluate(() => document.getElementById("Leads_listView_row_1").getAttribute("data-recordurl"));
  console.log(recordURL);
  //go to record
  await page.goto("https://ema.agentcrmlogin.com/" + recordURL);
  //get phone number
  const phoneNumberSelector = "#detailView > div > div.left-block.col-lg-4 > div.summaryView > div.summaryViewFields > div > table > tbody > tr:nth-child(4) > td.fieldValue > div > span.value.textOverflowEllipsis > a";
  ...
  const leadStatusTextSelector = "#detailView > div > div.left-block.col-lg-4 > div.summaryView > div.summaryViewFields > div > table > tbody > tr:nth-child(5) > td.fieldValue > div > span.value.textOverflowEllipsis > span";
  await page.waitForSelector(leadStatusTextSelector);
  await page.click(leadStatusTextSelector);
  const leadStatusSelector = "#detailView > div > div.left-block.col-lg-4 > div.summaryView > div.summaryViewFields > div > table > tbody > tr:nth-child(5) > td.fieldValue > div > span.action > .editAction";
  ...
  await page.keyboard.type("Call Back");


  //const recordURL = await page.$eval("#Leads_listView_row_1", el => ).getAttribute("data-recordurl");
/*   for (i=1; i < 2; i++) {
